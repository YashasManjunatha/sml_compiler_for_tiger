...
structure Translate : TRANSLATE = struct 
  ...
  fun str(NODE({parent, frame, ...})) = str(parent) ^ " -> [" ^ Int.toString(!(#bottomOffset frame)) ^ "] " ^ Symbol.name(Frame.name frame)
    | str(ROOT) = "ROOT"
  ...
  fun procEntryExit{level=ROOT, ...} = ERR.error 0 "Cannot declare a function at ROOT"
    | procEntryExit{level=NODE{frame, ...}, body=body} = 
        let 
          val fullFunc  = Frame.procEntryExit(
                          frame, 
  ...
  fun intIR(n) = Ex(R.CONST n)

  fun transCall(l, args) = Ex(R.CALL(R.NAME l, map unEx args))

  fun nilIR(n) = Ex(R.MEM (R.CONST 0))
