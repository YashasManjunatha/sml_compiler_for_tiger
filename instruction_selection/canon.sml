(* Does not seem  to make false label follow cjumps? *)
...
struct
 ...
 let
  ...
  and do_stm(T.SEQ el) = (case el of 
                              [] => T.EXP(T.CONST 0)
                            | [x] => do_stm x
                            |  l => T.SEQ(map do_stm l))
               (* T.SEQ(do_stm a % do_stm b) *)
  ...
  (* Modified version for our seq list *)
  fun linear (T.SEQ(seqlist)) = List.concat(map linear seqlist)
    | linear (a) = [a]
