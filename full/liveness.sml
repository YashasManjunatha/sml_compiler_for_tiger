...
sig
  datatype igraph =
  IGRAPH of { graph: Temp.temp Graph.graph,
              tnode: Temp.temp -> Temp.temp Graph.node,
              gtemp: Temp.temp Graph.node -> Temp.temp,
              moves: (Temp.temp Graph.node * Temp.temp Graph.node) list}
  val interferenceGraph : Flow.flowgraph -> igraph * (Assem.instr Graph.node -> Temp.temp list)
...
struct
  ...
  let 
    ...
    fun nodeStr (nid, data) = 
      "Node " ^ Int.toString nid ^ ": " ^ MipsFrame.getRegName(data) 
  in
    Graph.printGraphToOut os formatTempNode graph
  end
  ...
  fun printLivenessMap(liveIn, liveOut, control) = 
    let
      fun list2string (tempList) = 
        foldl 
          (fn(t, s) => s ^ ", " ^ (MipsFrame.getRegName t))
          ""
          tempList
  ...
  fun computeLiveness (def, use, liveIn, liveOut, nodes) = 
    let
      ...
      fun applyDataflowEquations(i, succ_ids, liveInArg, liveOutArg, nothingChangeArg) =
       let
        (* val () = print("Applying DataFlow to node " ^ Int.toString i ^ "\n")
         * *)
        ...
        fun templist2str (l) = 
          String.concat(map (fn(t) => ", " ^ F.getRegName(t)) l) ^ "\n"
       in
         (
    ...
    in
      (
      (*print("Length of incoming Nodes: " ^ Int.toString (List.length
       * nodes));*)
  ...
  fun interferenceGraph (flowgraph) = 
    let
      ...
      (* val () = print("Length of nodes for interference graph: " ^
       * Int.toString (List.length nodes) ^ "\n") *)
      ...
      val igraph = 
        let 
          val allTemps = Set.toList(Set.fromList(List.concat(G.NodeMap.listItems def) @
          List.concat(G.NodeMap.listItems use)))
          fun addTempToIgraph(t, g) = Graph.addNode(g, t, t)
          (*
           * val () = print(
              "All Temps: "^
              String.concat(( map (fn(t) =>  ", " ^ MipsFrame.getRegName t) allTemps) )
          )
          val () = print("\n") 
           *)

        in
          foldl addTempToIgraph Graph.empty allTemps
        end
      ...
      fun addInstructionNodeEdges((i, _, _, _), igr) =
        let 
          ...
          fun addSingleDefEdges ig (defReg) = 
            foldl 
              (fn(liveOutTemp, g) => (if liveOutTemp = defReg then g else Graph.addEdge(g, {from=defReg,
               to=liveOutTemp}) 
                  handle e => (
                    print ("Instruction " ^ Int.toString i ^ "\n"); 
                    print ("IGR Nodes: " ^ String.concatWith " " (map (fn (_, d, _, _)
                    => MipsFrame.getRegName d) (Graph.nodes igr)) ^ "\n");
      ...
      (*val () = Graph.printGraph formatTempNodeExcludeSpecial igraph'*)
      ...
      val moveList = 
        let
          ...
          fun getMoveTuple (Assem.MOVE{src, dst, ...}) =
                (Graph.getNode(igraph', src) handle e => raise e, 
                 Graph.getNode(igraph', dst) handle e => raise e)
      ...
      val return_igraph = IGRAPH{ graph = igraph',
              tnode = fn(t) => Graph.getNode(igraph', t),
              gtemp = fn(node) => Graph.nodeInfo(node),
              moves = moveList}
